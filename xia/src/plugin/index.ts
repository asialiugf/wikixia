import { EditorInterface, isEngine, isSafari, NodeInterface, Plugin, PluginOptions } from '@aomao/engine';
  ...
  onSlash = (event: KeyboardEvent) => {
    ...
    if ('' === text || ('/' === text && isSafari) || event.ctrlKey || event.metaKey) {
      ...
      if (range.collapsed) {
        ...
        const card = this.editor.card.insert(ToolbarComponent.cardName, {}, data) as ToolbarComponent<ToolbarValue>;
        card.setData(data);
