import { $, Card, isEngine, NodeInterface, isHotkey, CardType, isServer, CardValue } from '@aomao/engine';
...
class ToolbarComponent<V extends ToolbarValue = ToolbarValue> extends Card<V> {
      ...
      group.items.forEach((item: any) => {
        ...
        if (collapseItem) {
          items.push({
            ...
            disabled: collapseItem.onDisabled ? collapseItem.onDisabled() : !this.editor.command.queryEnabled(name)
          });
        }
      });
      data.push({
  ...
  handleInput() {
    ...
    const content = this.keyword?.get<HTMLElement>()?.innerText.replace(/[\r\n]/g, '') || '';
    // 内容为空
    if (content === '') {
      this.component?.remove();
      card.remove(this.id);
