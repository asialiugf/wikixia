export default {
  'Rule name': 'Rule name',
  controller: 'controller',
  'data sheet': 'data sheet',
  'Data table primary key': 'Data table primary key',
  'Sensitive fields': 'Sensitive fields',
  'Modifying monitoring': 'Modifying monitoring',
  'The rule name helps to identify the modified data later': 'The rule name helps to identify the modified data later',
  'The data listening mechanism will monitor the modification operations under this controller':
    'The data listening mechanism will monitor the modification operations under this controller',
  'Corresponding data sheet': 'Corresponding data sheet',
  'Filling in field notes helps you quickly identify fields later':
    'Filling in field notes helps you quickly identify fields later'
};
